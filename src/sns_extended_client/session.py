...
def _create_s3_put_object_params(self, encoded_body, topicArn):
  return {
    'ACL': 'private',
    'Body': encoded_body,
...
def _determine_payload(self, topicArn, message_attributes, message_body, message_structure):
  ...
  if self.large_payload_support and (self.always_through_s3 or self._is_large_message(message_attributes, encoded_body)):
    ...
    if RESERVED_ATTRIBUTE_NAME in message_attributes:
      raise Exception('Message attribute name ' + RESERVED_ATTRIBUTE_NAME + 'is reserved for use by SNS extended client.')
